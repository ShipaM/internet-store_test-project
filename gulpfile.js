...
const pug             = require('gulp-pug');//подключаем  gulp-pug
...
gulp.task('pug', function () {
  return gulp.src(path.src.pug)//путь к файлам которые мы обрабатываем
    .pipe(pug({// обработчик gulp
      pretty:true//эта команда дает нам не минифицированный html файл
    }))
    .pipe(gulp.dest(path.dist.html))// путь куда выгружаем файл
...
gulp.task('dist', shell.task([
    ...
    'gulp pug',//3. запускаем task html
